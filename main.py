...
_user_last_id = 0
_telegrambot_token = '5721196656:AAFEmzNbzoFIdgupBVRC87U8H6ea2gc__4I'
_telegram_chat_id = '628169526' # you can get this in @cid_bot bot.
def getUsers():
...
class AccessChecker(threading.Thread):
    ...
    def run(self):
        ...
        while users_list.count:
            ...
            requests.get(f'https://api.telegram.org/bot{_telegrambot_token}/sendMessage?chat_id={_telegram_chat_id}&text={user_remark}%20%7c%20connection%3d{connection_count}&20%7c%20UserPort%3d{user_port}')
            print(f"inbound with port {user_port} connection {connection_count}")
            #else:
            time.sleep(3600000)

init();
schedule.every(60).minutes.do(checkNewUsers)
while True:
